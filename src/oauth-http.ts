...
export function createOAuthHttp(options: Options) {
  ...
  const canonicalResource = new URL("/mcp", options.issuer).href;
  ...
  /** The consent form posts to /oauth/consent, which answers 303 to the client's redirect URI. Browsers
   *  enforce form-action across that redirect, so the consent page must name the client origin as well —
   *  with a bare 'self' the browser silently drops the approval and the flow dead-ends. */
  const pageHeaders = (res: ServerResponse, formAction = "'self'") => { res.setHeader("cache-control", "no-store"); res.setHeader("content-security-policy", `default-src 'none'; style-src 'unsafe-inline'; form-action ${formAction}; frame-ancestors 'none'; base-uri 'none'`); res.setHeader("x-frame-options", "DENY"); };
  ...
  return {
    ...
    async handle(request: IncomingMessage, response: ServerResponse, url: URL): Promise<boolean> {
      ...
      if (url.pathname === "/register" && request.method === "POST") { if (!options.enableDcr) return json(response, 403, { error: "access_denied" }), true; try { const raw = await bodyText(request, options.maxBodyBytes); const body = JSON.parse(request.headers["content-type"]?.includes("application/json") ? raw : new URLSearchParams(raw).get("client_metadata") ?? "{}"); const client = provider.registerClient(body, options.requestKey(request)); const name = typeof body.client_name === "string" && body.client_name.length <= 256 ? { client_name: body.client_name } : {}; return json(response, 201, { ...client, ...name, client_id_issued_at: Math.floor(Date.now() / 1000), grant_types: ["authorization_code", "refresh_token"], response_types: ["code"], scope: options.scopes.join(" ") }), true; } catch (error) { if (error instanceof OAuthClientQuotaError) { response.setHeader("retry-after", "3600"); return json(response, 429, { error: "temporarily_unavailable", error_description: error.message }), true; } return authorizationError(response, error instanceof Error ? error.message : "Invalid client metadata"), true; } }
      if (url.pathname === "/authorize" && request.method === "GET") {
        const clientId = url.searchParams.get("client_id") ?? ""; const redirectUri = url.searchParams.get("redirect_uri") ?? ""; const codeChallenge = url.searchParams.get("code_challenge") ?? ""; const requestedScopes = (url.searchParams.get("scope") || options.scopes.join(" ")).split(/\s+/).filter(Boolean); const resource = url.searchParams.get("resource") || canonicalResource; const state = url.searchParams.get("state") ?? undefined;
      ...
      if (url.pathname === "/token" && request.method === "POST") { const body = await form(request, options.maxBodyBytes); try { const resource = body.get("resource") || canonicalResource; const scopes = (body.get("scope") ?? "").split(/\s+/).filter(Boolean); const tokens = body.get("grant_type") === "authorization_code" ? provider.exchangeCode({ clientId: body.get("client_id") ?? "", code: body.get("code") ?? "", redirectUri: body.get("redirect_uri") ?? "", codeVerifier: body.get("code_verifier") ?? "", resource }) : body.get("grant_type") === "refresh_token" ? provider.refresh({ clientId: body.get("client_id") ?? "", refreshToken: body.get("refresh_token") ?? "", scopes: scopes.length ? scopes : undefined, resource }) : null; if (!tokens) return authorizationError(response, "Unsupported grant type"), true; return json(response, 200, tokens), true; } catch (error) { if (error instanceof OAuthGrantQuotaError) { response.setHeader("retry-after", "60"); return json(response, 429, { error: "temporarily_unavailable", error_description: error.message }), true; } return json(response, 400, { error: "invalid_grant", error_description: error instanceof Error ? error.message : "Token exchange failed" }), true; } }
